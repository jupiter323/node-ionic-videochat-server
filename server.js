...
// connect to the database/127.0.0.1:27017
// mongo.connect(process.env.MONGODB_URI || 'mongodb://localhost/ionic-video-chat', (err, d) => {
mongo.connect(process.env.MONGODB_URI || 'mongodb://jupiter123:jupiter12345@ds239029.mlab.com:39029/ionic-video-chat', (err, d) => {
	if (err) {
...
io.on('connection', socket => {
	...
	*/
	...
	socket.on('auth', token => {
		...
		if (!decoded || !decoded.email || decoded.email == 'undefined') {
	...
	socket.on('sendMessage', (userEmail, message) => {
		let currentUser = checkUser();
		if (!currentUser) return;
		console.log("user iD",userEmail, currentUser);
		var contact;
		users.forEach(usr => {
		...
		console.log('sendMessage: from ' + currentUser.email + ' to ' + contact.email);
		io.to(contact.socket).emit('messageReceived', currentUser, message);
	});
